;;; linux.lisp - Linux input event codes
;;;
;;; This is a port of all the constants defined on Linux in
;;; </usr/include/linux/input-event-codes.h>. Documentation is preserved where
...
(defpackage #:xyz.shunter.input-event-codes
  ...
  (:export
    ...
    +key-ab+ +key-addressbook+ +key-again+ +key-all-applications+ +key-als-toggle+
    +key-alterase+ +key-angle+ +key-apostrophe+ +key-appselect+ +key-archive+
    +key-aspect-ratio+ +key-assistant+ +key-attendant-off+ +key-attendant-on+
    +key-attendant-toggle+ +key-audio+ +key-audio-desc+
    +key-autopilot-engage-toggle+ +key-aux+ +key-b+ +key-back+ +key-backslash+
    ...
    +key-brightness-max+ +key-brightness-menu+ +key-brightness-min+
    +key-brightness-toggle+ +key-brightness-zero+ +key-brightnessdown+
    +key-brightnessup+ +key-brl-dot1+ +key-brl-dot10+ +key-brl-dot2+
    +key-brl-dot3+ +key-brl-dot4+ +key-brl-dot5+ +key-brl-dot6+ +key-brl-dot7+
    +key-brl-dot8+ +key-brl-dot9+ +key-buttonconfig+ +key-c+ +key-calc+
    +key-calendar+ +key-camera+ +key-camera-down+ +key-camera-focus+
    +key-camera-left+ +key-camera-right+ +key-camera-up+ +key-camera-zoomin+
    +key-camera-zoomout+ +key-cancel+ +key-capslock+ +key-cd+ +key-channel+
    +key-channeldown+ +key-channelup+ +key-chat+ +key-clear+ +key-clearvu-sonar+
    ...
    +key-del-line+ +key-delete+ +key-deletefile+ +key-dictate+ +key-digits+
    +key-direction+ +key-directory+ +key-display-off+ +key-displaytoggle+
    +key-documents+ +key-dollar+ +key-dot+ +key-down+ +key-dual-range-radar+
    +key-dvd+ +key-e+ +key-edit+ +key-editor+ +key-ejectcd+ +key-ejectclosecd+
    +key-email+ +key-emoji-picker+ +key-end+ +key-enter+ +key-epg+ +key-equal+
    +key-esc+ +key-euro+ +key-exit+ +key-f+ +key-f1+ +key-f10+ +key-f11+ +key-f12+
    +key-f13+ +key-f14+ +key-f15+ +key-f16+ +key-f17+ +key-f18+ +key-f19+ +key-f2+
    +key-f20+ +key-f21+ +key-f22+ +key-f23+ +key-f24+ +key-f3+ +key-f4+ +key-f5+
    +key-f6+ +key-f7+ +key-f8+ +key-f9+ +key-fastforward+ +key-fastreverse+
    +key-favorites+ +key-file+ +key-finance+ +key-find+ +key-first+
    +key-fishing-chart+ +key-fn+ +key-fn-1+ +key-fn-2+ +key-fn-b+ +key-fn-d+
    +key-fn-e+ +key-fn-esc+ +key-fn-f+ +key-fn-f1+ +key-fn-f10+ +key-fn-f11+
    +key-fn-f12+ +key-fn-f2+ +key-fn-f3+ +key-fn-f4+ +key-fn-f5+ +key-fn-f6+
    +key-fn-f7+ +key-fn-f8+ +key-fn-f9+ +key-fn-right-shift+ +key-fn-s+
    +key-forward+ +key-forwardmail+ +key-frameback+ +key-frameforward+ +key-front+
    +key-full-screen+ +key-g+ +key-games+ +key-goto+ +key-graphicseditor+
    +key-grave+ +key-green+ +key-h+ +key-hangeul+ +key-hanguel+ +key-hangup-phone+
    +key-hanja+ +key-help+ +key-henkan+ +key-hiragana+ +key-home+ +key-homepage+
    +key-hp+ +key-i+ +key-images+ +key-info+ +key-ins-line+ +key-insert+ +key-iso+
    +key-j+ +key-journal+ +key-k+ +key-katakana+ +key-katakanahiragana+
    +key-kbd-layout-next+ +key-kbd-lcd-menu1+ +key-kbd-lcd-menu2+
    +key-kbd-lcd-menu3+ +key-kbd-lcd-menu4+ +key-kbd-lcd-menu5+ +key-kbdillumdown+
    ...
    +key-mail+ +key-mark-waypoint+ +key-max+ +key-media+ +key-media-repeat+
    +key-media-top-menu+ +key-memo+ +key-menu+ +key-messenger+ +key-mhp+
    +key-micmute+ +key-min-interesting+ +key-minus+ +key-mode+ +key-move+
    +key-mp3+ +key-msdos+ +key-muhenkan+ +key-mute+ +key-n+ +key-nav-chart+
    +key-nav-info+ +key-new+ +key-news+ +key-next+ +key-next-element+
    +key-next-favorite+ +key-nextsong+ +key-notification-center+ +key-numeric-0+
    +key-numeric-1+ +key-numeric-11+ +key-numeric-12+ +key-numeric-2+
    +key-numeric-3+ +key-numeric-4+ +key-numeric-5+ +key-numeric-6+
    +key-numeric-7+ +key-numeric-8+ +key-numeric-9+ +key-numeric-a+
    +key-numeric-b+ +key-numeric-c+ +key-numeric-d+ +key-numeric-pound+
    +key-numeric-star+ +key-numlock+ +key-o+ +key-ok+ +key-onscreen-keyboard+
    +key-open+ +key-option+ +key-p+ +key-pagedown+ +key-pageup+ +key-paste+
    +key-pause+ +key-pause-record+ +key-pausecd+ +key-pc+ +key-phone+
    +key-pickup-phone+ +key-play+ +key-playcd+ +key-player+ +key-playpause+
    +key-power+ +key-power2+ +key-presentation+ +key-previous+
    +key-previous-element+ +key-previoussong+ +key-print+
    +key-privacy-screen-toggle+ +key-prog1+ +key-prog2+ +key-prog3+ +key-prog4+
    +key-program+ +key-props+ +key-pvr+ +key-q+ +key-question+ +key-r+
    +key-radar-overlay+ +key-radio+ +key-record+ +key-red+ +key-redo+
    ...
    +key-send+ +key-sendfile+ +key-setup+ +key-shop+ +key-shuffle+
    +key-sidevu-sonar+ +key-single-range-radar+ +key-slash+ +key-sleep+ +key-slow+
    +key-slowreverse+ +key-sos+ +key-sound+ +key-space+ +key-spellcheck+
    +key-sport+ +key-spreadsheet+ +key-stop+ +key-stop-record+ +key-stopcd+
    +key-subtitle+ +key-suspend+ +key-switchvideomode+ +key-sysrq+ +key-t+
    +key-tab+ +key-tape+ +key-taskmanager+ +key-teen+ +key-text+ +key-time+
    +key-title+ +key-touchpad-off+ +key-touchpad-on+ +key-touchpad-toggle+
    +key-traditional-sonar+ +key-tuner+ +key-tv+ +key-tv2+ +key-twen+ +key-u+
    ...
    +sw-keypad-slide+ +sw-lid+ +sw-linein-insert+ +sw-lineout-insert+
    +sw-machine-cover+ +sw-max+ +sw-microphone-insert+ +sw-mute-device+
    +sw-pen-inserted+ +sw-radio+ +sw-rfkill-all+ +sw-rotate-lock+ +sw-tablet-mode+
    +sw-videoout-insert+ +syn-cnt+ +syn-config+ +syn-dropped+ +syn-max+
    +syn-mt-report+ +syn-report+))
...
(defconstant +key-all-applications+ 204
             "AC Desktop Show All Applications")
(defconstant +key-dashboard+ +key-all-applications+)
...
(defconstant +key-switchvideomode+ 227
...
(defconstant +key-images+ #x1ba
             "AL Image Browser")
(defconstant +key-notification-center+ #x1bc
             "Show/hide the notification center")
(defconstant +key-pickup-phone+ #x1bd
             "Answer incoming call")
(defconstant +key-hangup-phone+ #x1be
...
(defconstant +key-fn-right-shift+ #x1e5)
...
(defconstant +key-emoji-picker+ #x249
             "Show/hide emoji picker (HUTRR101)")
(defconstant +key-dictate+ #x24a
...
(defconstant +key-next-element+ #x27b
             "Move the focus to the next or previous user controllable element within a UI container")
(defconstant +key-previous-element+ #x27c)

(defconstant +key-autopilot-engage-toggle+ #x27d
             "Toggle Autopilot engagement")

;; Shortcut Keys
(defconstant +key-mark-waypoint+ #x27e)
(defconstant +key-sos+ #x27f)
(defconstant +key-nav-chart+ #x280)
(defconstant +key-fishing-chart+ #x281)
(defconstant +key-single-range-radar+ #x282)
(defconstant +key-dual-range-radar+ #x283)
(defconstant +key-radar-overlay+ #x284)
(defconstant +key-traditional-sonar+ #x285)
(defconstant +key-clearvu-sonar+ #x286)
(defconstant +key-sidevu-sonar+ #x287)
(defconstant +key-nav-info+ #x288)
(defconstant +key-brightness-menu+ #x289)
...
;; It was used by HID as ABS-MISC+6 and userspace needs to detect if
;; the next ABS-* event is correct or is just ABS-MISC + n.
...
(defconstant +sw-pen-inserted+ #x0f
             "set = pen inserted")
(defconstant +sw-machine-cover+ #x10
             "set = cover closed")
(defconstant +sw-max+ #x10)
